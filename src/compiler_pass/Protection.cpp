...
struct VProtectionPass : public FunctionPass {
  ...
  StringRef getPassName() const override { return "VProtectionPass"; }

  bool runOnFunction(Function &F) override {
    if (!F.isIntrinsic() && !isInternalFunction(F.getName()) &&
  ...
  static bool isInternalFunction(StringRef name) {
    static StringSet<> ifunc = {
        __GEP_CHECK,    __BITCAST_CHECK,   __REPORT_STATISTIC,
        __REPORT_ERROR, __GET_CHUNK_RANGE, __ESCAPE,
    };
  ...
  void bindRuntime() {
    ...
    M->getOrInsertFunction(
        __GEP_CHECK,
        FunctionType::get(
            int32Type, {voidPointerType, voidPointerType, int64Type}, false));

    M->getOrInsertFunction(
        __BITCAST_CHECK,
        FunctionType::get(int32Type, {voidPointerType, int64Type}, false));

    M->getOrInsertFunction(__REPORT_STATISTIC,
                           FunctionType::get(voidType, {}, false));

    M->getOrInsertFunction(__REPORT_ERROR,
                           FunctionType::get(voidType, {}, false));

    M->getOrInsertFunction(
        __GET_CHUNK_RANGE,
        FunctionType::get(int64Type, {int64Type, int64PointerType}, false));

    M->getOrInsertFunction(
        __ESCAPE, FunctionType::get(int32Type,
                                    {voidPointerType, voidPointerType}, false));

    M->getOrInsertFunction(__STRCPY_CHECK,
                           FunctionType::get(voidPointerType,
    ...
    M->getOrInsertFunction(__STRCAT_CHECK,
                           FunctionType::get(voidPointerType,
                                             {voidPointerType, voidPointerType},
                                             false));
  ...
  void report() {
    ...
    if (bitcastRuntimeCheck > 0 || bitcastPartialCheck > 0 ||
        bitcastBuiltinCheck > 0) {
      dbgs() << "    [BitCast]\n";
  ...
  void addBuiltinCheck(Instruction *I, Value *Cond) {
    ...
    */
    ...
    IRBuilder<> irBuilder(
        SplitBlockAndInsertIfThen(Cond, fetchBestInsertPoint(I), false));
  ...
  void addPartialCheck(Value *V, SmallVector<Instruction *, 16> *S) {
    ...
    if (MaybeStack(V)) {
      ...
      irBuilder.SetInsertPoint(
          SplitBlockAndInsertIfThen(valueNotOnStack, InsertPoint, false));
      auto if_end = irBuilder.CreateCall(M->getFunction(__GET_CHUNK_RANGE),
                                         {ptr, base_ptr});
      auto if_base = irBuilder.CreateLoad(base_ptr);

      irBuilder.SetInsertPoint(InsertPoint);
      PHINode *base_phi = irBuilder.CreatePHI(int64Type, 2);
      base_phi->addIncoming(
          ConstantInt::get(int64Type, 0),
          dyn_cast<Instruction>(valueNotOnStack)->getParent());
      base_phi->addIncoming(if_base, if_base->getParent());

      PHINode *end_phi = irBuilder.CreatePHI(int64Type, 2);
      end_phi->addIncoming(ConstantInt::get(int64Type, 0x1000000000000),
                           dyn_cast<Instruction>(valueNotOnStack)->getParent());
      ...
      end = irBuilder.CreateCall(M->getFunction(__GET_CHUNK_RANGE),
                                 {ptr, base_ptr});
    ...
    for (auto I : *S) {
      ...
      irBuilder.SetInsertPoint(
          SplitBlockAndInsertIfThen(Cond, InsertPoint, false));
      irBuilder.CreateCall(M->getFunction(__REPORT_ERROR), {});
    }
  }
#endif
  void addGepRuntimeCheck(Instruction *I) {
    ...
    */
    ...
    Value *base =
        irBuilder.CreatePointerCast(gep->getPointerOperand(), voidPointerType);
    ...
    if (MaybeStack(source[I])) {
      ...
      Value *value =
          irBuilder.CreatePtrToInt(gep->getPointerOperand(), int64Type);
  ...
  void addBitcastRuntimeCheck(Instruction *I) {
    ...
    */
    ...
    Value *size = irBuilder.getInt64(
        DL->getTypeAllocSize(bc->getDestTy()->getPointerElementType()));
  ...
  void addEscape(StoreInst *SI) {
    ...
    Value *locNotOnStack = IRB.CreateICmpULT(
        IRB.CreatePtrToInt(SI->getPointerOperand(), int64Type), rsp);
    cond = IRB.CreateAnd(cond, locNotOnStack);
#endif
    IRB.SetInsertPoint(SplitBlockAndInsertIfThen(cond, SI, false));
    IRB.CreateCall(
        M->getFunction(__ESCAPE),
  ...
  bool isHeapAddress(Value *Ptr, SmallSet<Value *, 16> &Visit) {
    ...
    if (isa<GlobalValue>(S) || isa<AllocaInst>(S) || isa<Constant>(S))
      return false;
  ...
  Value *getBoundsCheckCond(Instruction *Ptr, SizeOffsetEvalType &SizeOffset) {
    assert(Ptr->getType()->isPointerTy() &&
           "getBoundsCheckCond(): Ptr should be pointer type");

    uint32_t NeededSize =
        auxiliary.count(Ptr)
            ? 0
            : DL->getTypeAllocSize(Ptr->getType()->getPointerElementType());
    IRBuilder<TargetFolder> IRB(Ptr->getParent(), BasicBlock::iterator(Ptr),
                                TargetFolder(*DL));
      ...
      for (Instruction &I : BB) {
        if (isa<GetElementPtrInst>(I) || isa<BitCastInst>(I)) {
          ...
          if (SI->getValueOperand()->getType()->isPointerTy()) {
            ...
            if (!cast<PointerType>(SI->getValueOperand()->getType())
                     ->getElementType()
                     ->isStructTy()) {
              escapeOptimized++;
              continue;
            ...
            Instruction *ptr =
                dyn_cast_or_null<Instruction>(SI->getValueOperand());
            ...
            Instruction *loc =
                dyn_cast_or_null<Instruction>(SI->getPointerOperand());
            if (loc && source.count(loc) &&
                (isa<AllocaInst>(source[loc]) ||
  ...
  void addAuxInstruction(CallInst *CI, Value *Ptr, Value *Len) {
    ...
    auto GEP =
        irBuilder.CreateGEP(BC, irBuilder.CreatePointerCast(Len, int64Type));
  ...
  Instruction *dependenceOptimize(Value *key,
                                  SmallVector<Instruction *, 16> *value) {
    Instruction *InsertPoint = nullptr;
    ...
    IRBuilder<TargetFolder> irBuilder(
        InsertPoint->getParent(),
        BasicBlock::iterator(InsertPoint->getIterator()), TargetFolder(*DL));
